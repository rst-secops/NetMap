...
function buildDeviceBlocks(nodes: DcNode[]): string[] {
  let usedChars = 0;
  const blocks: string[] = [];
  for (const node of nodes) {
    const resultsStr = node.results ? JSON.stringify(node.results, null, 2) : "(no output)";
    const header = `Device: ${node.nodeDisplayName} (${node.nodeType})\nHost: ${node.host}\n--- Output ---\n`;
    const footer = `\n---\n`;
    const remaining = CHAR_BUDGET - usedChars - header.length - footer.length;
    let body: string;
    if (remaining <= 0) {
      body = "[TRUNCATED]";
    ...
    blocks.push(header + body + footer);
...
${vlanRule}`;
  ...
  const deviceBlocks = buildDeviceBlocks(nodes);
  const userMessage =
